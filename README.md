...
To have the required Python version and dependencies installed in an isolated environment:
...
```bash
docker buildx build --platform linux/amd64 -t registry.agrospai.udl.cat/library/provider:latest -t registry.agrospai.udl.cat/library/provider:v2.1.3 --push .
```
